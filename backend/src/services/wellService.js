        ...
        try {
         ...
         RETURNING *`,
           ...
           RETURNING id`,
            ...
            // 5. Bulk insert measurements IN BATCHES
            const BATCH_SIZE = 20000; // Safe batch size (20k params = ~6,666 measurements)
            const depthMnemonic = parsedData.curves[0].mnemonic;

            // Prepare all measurement data
            ...
            // Insert in batches
            console.log(`Inserting ${allMeasurements.length} measurements in batches...`);

            for (let i = 0; i < allMeasurements.length; i += BATCH_SIZE / 3) {
                const batch = allMeasurements.slice(i, i + (BATCH_SIZE / 3));
